...
RUNNABLE=`cat ${DESKTOP_FILE} | grep Exec | cut -f2 -d= | cut -f1 -d " " | head -n1`
...
echo "export XDG_DATA_DIRS=\${APPDIR}/usr/share/" | tee -a ${WRAPPER_PATH}
echo "\${APPDIR}/usr/bin/${RUNNABLE}" | tee -a ${WRAPPER_PATH}
echo "echo \"APPDIR: \${APPDIR}\"" | tee -a ${WRAPPER_PATH}
echo "echo \"XDG_DATA_DIRS: \${XDG_DATA_DIRS}\"" | tee -a ${WRAPPER_PATH}
